    ...
    <div class="bg-zinc-100 rounded-lg mt-4 px-1 pt-4 min-h-screen min-w-screen backdrop-blur-md bg-white/65 border border-white/30 shadow-lg overflow-scroll">
        ...
        <section class="w-full flex justify-between items-center">
          ...
          <div class="flex gap-4">
            <a href="{{ route('employee.createKantor', ['from' => 'kantor 1','bulan' => request('bulan'),'tahun' => request('tahun')]) }}" class="flex items-center my-4 px-4 py-1 bg-zinc-800 text-white rounded-md hover:bg-zinc-900"><i class="fas fa-plus mr-2"></i>Buat baru</a>
            <a href="{{ route('print.kantor1.filtered',['bulan' => request('bulan'),'tahun' => request('tahun'),'kantor' => 'kantor 1']) }}" class="bg-zinc-100 border border-zinc-800 flex items-center my-4 px-4 py-1 text-zinc-900 rounded-md hover:bg-zinc-200"><i class="fas fa-print mr-2"></i>Print Dokumen</a>    
        ...
        <div class="flex flex-wrap space-x-2 border-b border-b-gray-300 border-t border-gray-300 py-2">
          ...
          @foreach ($months as $name)
            <a href="{{ route('filterbymonth.kantor', ['bulan' => $name, 'tahun' => $currentYear, 'kantor' => 'kantor 1']) }}" class="text-sm px-4 py-1 border rounded hover:shadow-md {{ request('bulan') == $name ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-800' }}"> <i class="fas fa-calendar-alt text-sm mr-1"></i>
                {{ $name }}
            </a>
          @endforeach
        </div>
        <div class="bg-gray-100">
            ...
            <table class="min-w-full table-auto border-collapse text-[0.8rem]">
                <thead class="text-zinc-900">
                    <tr>
                      <th  class="py-2 w-5 border border-zinc-300 bg-zinc-100">No.</th>
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 w-[200px]">Nama</th>
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 w-[150px]">Masuk kerja</th>
                      <!-- Gaji Pokok with 3 sub-columns -->
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 text-center">Gaji Pokok (Rp.)</th>
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 text-center">Hari Kerja</th>
                      <!-- Tunjangan -->
                      <!-- Sub-columns for tunjangan -->
                      <th class="py-2 border border-zinc-300 bg-zinc-100 w-[150px]">Uang Makan (Rp.)</th>
                      <!-- Jumlah Kotor -->
                      <th  class="py-2 border border-zinc-300 bg-zinc-100">Total Gaji (Rp.)</th>
                      {{-- <th class="py-2 border border-zinc-300 bg-zinc-100 w-[150px]">BPJS</th> --}}
                        <!-- Sub-columns for Potongan -->
                        <th class="py-2 border border-zinc-300 bg-zinc-100 w-[120px]">Kredit/kasbon (Rp.)</th>
                      ...
                      <!-- Jumlah Bersih -->
                      <th  class="py-2 border border-zinc-300 bg-zinc-100">Jumlah Gaji Bersih (Rp.)</th>
                      <!-- TTD -->
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 w-[60px]">TTD</th>
                      <th  class="py-2 border border-zinc-300 bg-zinc-100 w-[50px]"></th>
                    </tr>
                </thead>
                <tbody>
                    @php $no = 1; @endphp
